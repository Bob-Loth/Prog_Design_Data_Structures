...
// TO DO -- write this function
  ...
  toDo.push(iStart);
  while (!toDo.empty()) {
	  int currINode = toDo.front(); //node under consideration
	  toDo.pop();
	  for (it = database[currINode].neighbors.begin(); it != database[currINode].neighbors.end(); ++it) {
		  //for all neighbors of the node under consideration
		  if (database[it->first].isVisited == true) continue; // if neighbor has been visited, skip
		  else {
			  database[it->first].cost = 1 + database[currINode].cost; // set neighbor cost to 1 + current node cost
			  database[it->first].prev = currINode;
			  toDo.push(it->first);
			  database[it->first].isVisited = true;
		  }
		  if (it->first == iEnd) {
			  while (!toDo.empty()) toDo.pop(); // clear toDolist
		  }
	  }
  }
  result.second = database[iEnd].cost; //get total cost
  int index = iEnd;
  while (index != iStart){
	  result.first.push(index); //get last to first, on the stack
	  index = database[index].prev; //go back
  }
  result.first.push(iStart); //get first
  return result;
}
